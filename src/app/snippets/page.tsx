...
//   try {
//     // Build optimized query with simplified Supabase syntax
...
//     // Transform data safely with proper typing - handle profiles array
...
//         author: snippet.profiles && snippet.profiles.length > 0 ? {
...
//     // Convert SearchParams to the format expected by ClientSortFilters
//     const searchParamsForClient: Record<string, string | string[] | undefined> = {
//       ...(query && { q: query }),
//       ...(language && { language }),
//       ...(tag && { tag }),
//       ...(sort && { sort }),
...
//         ) : (
//           <>
//             <ClientSortFilters currentSort={sort} searchParams={searchParamsForClient} />
...
async function SnippetsContent({ searchParams }: SnippetsPageProps) {
  ...
  const language = getStringParam(params.language);
  ...
  try {
    ...
    const searchParamsForClient: Record<string, string | string[] | undefined> = {
      ...
      ...(tag && { tag }),
      ...(sort && { sort }),
      ...(pageParam && pageParam !== "1" && { page: pageParam }),
    };

    // Create simple params object for pagination
    const simpleParams = {
      ...(query && { q: query }),
      ...(language && { language }),
      ...(tag && { tag }),
