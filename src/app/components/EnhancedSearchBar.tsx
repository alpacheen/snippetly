// src/app/components/EnhancedSearchBar.tsx
"use client";

import { useState, useEffect, useRef } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import { Search, Filter, X, Clock, Code, Hash } from 'lucide-react';
import { supabase } from '@/lib/supabase';

interface SearchSuggestion {
  type: 'language' | 'tag' | 'recent' | 'snippet';
  value: string;
  count?: number;
  id?: string;
}
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      if (query.length === 0) {
        ...
        const recent = getRecentSearches();
        setSuggestions(recent.map(search => ({ type: 'recent', value: search })));
        return;
      }
          ...
          {loading ? (
            ...
            <>
              {query.length === 0 && suggestions.some(s => s.type === 'recent') && (
                <div className="flex items-center justify-between p-3 border-b border-textSecondary">
                  <span className="text-sm font-medium text-textSecondary">Recent Searches</span>
                  <button
