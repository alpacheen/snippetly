  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const [snippetQuery, languageQuery] = await Promise.allSettled([
          supabase
            .from('snippets')
            .select('id, title, language')
          ...
          supabase
            .from('snippets')
            .select('language')
            .ilike('language', `%${debouncedQuery}%`)
            .limit(3)
            .abortSignal(abortControllerRef.current.signal)
        ]);

        // Process snippet results
        if (snippetQuery.status === 'fulfilled' && snippetQuery.value.data) {
          snippetQuery.value.data.forEach(snippet => {
            suggestions.push({
              type: 'snippet',
              value: snippet.title,
              id: snippet.id
        ...
        if (languageQuery.status === 'fulfilled' && languageQuery.value.data) {
          const uniqueLanguages = [...new Set(languageQuery.value.data.map(l => l.language))];
          uniqueLanguages.forEach(lang => {
            suggestions.push({
              type: 'language',
              value: lang
        ...
        // Cache the results
        suggestionCache.set(cacheKey, suggestions.slice(0, 8));
        setSuggestions(suggestions.slice(0, 8));
      } catch (error) {
